...
router.put("/tasks/:taskId/status", async (req, res) => {
  try {
    ...
    try {
      ...
      // Use specific notification methods for delayed and completed statuses
      if (status === 'delayed') {
        await notificationService.notifyTaskDelayed(
          fullyPopulatedTask,
          delayReason || 'No reason provided',
          req.user
        );
        console.log('✅ Task delayed notifications sent successfully');
      } else if (status === 'completed') {
        ...
        // Use general status change notification for other statuses
        ...
        console.log('✅ Task status update notifications sent successfully');
      }
