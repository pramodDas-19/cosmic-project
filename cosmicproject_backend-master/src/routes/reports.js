...
router.post('/generate', async (req, res) => {
  ...
  if (type === 'Project Summary' && format === 'excel') {
  ...
  projects.forEach(p => {
    worksheet.addRow({
      projectName: p.siteName || p.name || 'N/A',
      status: p.status || '-',
  ...
  res.setHeader(
    'Content-Disposition',
    'attachment; filename="Project_Summary_report.xlsx"'
  );

  await workbook.xlsx.write(res);

  // 🔴 CRITICAL: END AND EXIT
  res.end();
  return;
}


    // ⬇️ PDF LOGIC CONTINUES UNCHANGED BELOW
