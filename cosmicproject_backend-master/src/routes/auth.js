...
// Import LoginSession model
let LoginSession;
try {
  LoginSession = require('../models/LoginSession');
} catch (error) {
  console.error('Error importing LoginSession model:', error.message);
}
  ...
  try {
    ...
    if (LoginSession) {
      try {
        ...
        console.error('⚠️ Failed to create login session:', sessionError.message);
      }
    } else {
      console.warn('⚠️ LoginSession model not available, skipping session creation');
    }
...
const logout = async (req, res) => {
  try {
    ...
    // Deactivate login session if token exists
    if (LoginSession && token) {
      try {
        const session = await LoginSession.findOne({ token, isActive: true });
        if (session) {
          await session.deactivate();
          console.log('✅ Login session deactivated for logout');
        }
      } catch (sessionError) {
        console.error('⚠️ Failed to deactivate login session:', sessionError.message);
