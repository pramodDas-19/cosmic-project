...
router.get('/stats', async (req, res) => {
  try {
    ...
    const pendingTasks = await Task.countDocuments({
      status: { $in: ['Assigned', 'In Progress', 'Delayed'] }
    });
    ...
    res.json({
      ...
      data: {
        ...
        completionRate: projectsCount > 0
          ? (completedProjects / projectsCount) * 100
          : 0,
...
router.post(
  ...
  [
    body('clientMobile').trim().notEmpty().withMessage('Client Contact is required'),
  ...
  async (req, res) => {
    try {
      ...
      // Send comprehensive notifications using new system
      try {
        const notificationService = require('../services/notificationService');
        console.log(`📧 Sending notifications for project creation:`);
        console.log(`   - Created by: ${req.user.name} (${req.user._id})`);
        console.log(`   - Assigned Manager: ${assignedManager}`);
        console.log(`   - Socket Server Available: ${!!global.socketServer}`);
        ...
        // Also emit socket event for real-time dashboard updates
        if (global.socketServer) {
          global.socketServer.emitProjectCreated(project, req.user);
        }
      } catch (e) {
...
router.put('/projects/:id', [
  ...
  try {
    ...
    // Send notifications for project update
    try {
      const notificationService = require('../services/notificationService');

      // Notify the superadmin who updated it (themselves)
      await notificationService.sendRealTimeNotification(req.user._id.toString(), {
        title: 'Project Updated Successfully',
        message: `You have successfully updated project "${project.siteName}"`,
        type: 'success',
        priority: 'medium',
        category: 'project',
        metadata: {
      ...
      // Notify the assigned manager if manager changed
      if (req.body.assignedManager && req.body.assignedManager !== project.assignedManager._id.toString()) {
        await notificationService.sendRealTimeNotification(req.body.assignedManager, {
          title: 'New Project Assigned',
          message: `You have been assigned to project: "${project.siteName}"`,
          type: 'info',
          priority: 'high',
          category: 'project',
          metadata: {
      ...
      } else if (project.assignedManager) {
        // Notify existing manager about update
        await notificationService.sendRealTimeNotification(project.assignedManager._id.toString(), {
          title: 'Project Updated',
          message: `Project "${project.siteName}" has been updated`,
          type: 'info',
          priority: 'medium',
          category: 'project',
          metadata: {
      ...
      if (global.socketServer) {
        global.socketServer.emitProjectUpdated(project, req.user);
      }
    } catch (e) {
      console.error('Notification error:', e.message);
    }
