...
router.post('/', [
  body('clientMobile').notEmpty().withMessage('Client Contact is required'),
  ...
  try {
    ...
    const {clientMobile ,description, status = 'Planning', priority = 'medium', clientName, location, siteName } = req.body;
      console.log("Data is from route projects.js.....:"+req.body);
    // if (useMockData) {
    //   const newProject = {
    ...
    try {
      ...
      if (global.socketServer) {
        global.socketServer.emitProjectCreated(newProject, req.user);
      }
    } catch (e) {
...
router.put('/:id', [
  ...
  try {
    ...
    try {
      ...
      // Use standardized notification method for project updates
      await notificationService.sendProjectUpdatedNotification(project, req.user);
...
router.patch('/:id/status', [
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Project status change notifications sent successfully');
      
      // Also emit socket event for real-time updates
      if (global.socketServer) {
        global.socketServer.emitProjectStatusChanged(project, status, req.user);
      }
    } catch (e) {
      console.error('❌ Notification error:', e.message);
    }
