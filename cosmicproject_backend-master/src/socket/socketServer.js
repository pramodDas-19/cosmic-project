...
class SocketServer {
  ...
  setupEventHandlers() {
    this.io.on('connection', (socket) => {
      ...
      // Store connected user (ensure userId is string for consistency)
      ...
      console.log(`   User ${userIdStr} joined room: ${userRoom}`);
  ...
  async sendNotificationToUser(userId, notification) {
    try {
      // Normalize userId to string to ensure consistent room naming
      const userIdStr = userId.toString();
      console.log(`Sending notification to user ${userIdStr}:`, notification);
      ...
      // Send to user via their personal room (user:userId) for instant delivery
      // This ensures notifications appear even if socket reconnects
      const roomName = `user:${userIdStr}`;
      this.io.to(roomName).emit('notification:new', notificationData);
      console.log(`✅ Socket notification sent to user ${userIdStr} via room ${roomName}`);
      
      // Also check if user is connected and log for debugging
      const socketId = this.connectedUsers.get(userIdStr);
      if (socketId) {
        console.log(`   User ${userIdStr} is connected (socket: ${socketId})`);
      } else {
        console.log(`   ⚠️ User ${userIdStr} is not currently connected, but notification saved to DB`);
      }
